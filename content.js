...
function toggleProgressBar() {
  const progressBarElements = document.querySelectorAll('.ytp-progress-bar-container');
...
var checkInterval = setInterval(waitForVideoLoad, 100);
...
document.addEventListener('keydown', function (event) {
  if (event.key === 'h') {
    ...
    chrome.runtime.sendMessage({ action: 'toggleOverlayKeyPressed', value: overlayVisible });
    if (overlayVisible === true) {
      progressBarVisible = true;
      chrome.runtime.sendMessage({ action: 'toggleProgressBarKeyPressed', value: progressBarVisible });
    }
  }
